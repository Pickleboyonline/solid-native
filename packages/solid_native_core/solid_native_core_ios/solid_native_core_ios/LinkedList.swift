public final class LinkedList<T> {
    ...
    public func index(at node: Node) -> Int {
        assert(head != nil, "List is empty")
        // assert(index >= 0, "index must be greater or equal to 0")
        ...
        guard node !== last else {
            return count - 1
        }
        
        var searchNode = node
        var count = 1
        while let next = searchNode.next {
            if next === node {
                return count
            }
            searchNode = next
    ...
    /// Function to insert a specific node before a node
    ///
    /// - Parameter node: The node to be deleted
    /// - Returns: The data value contained in the deleted node.
    public func insertBefore(newNode: Node, beforeNode: Node) -> Void {
        guard self.head !== beforeNode else {
            self.insert(newNode, at: 0)
            return
        }
        
        guard self.last !== beforeNode else {
            self.insert(newNode, at: count - 1)
            return
        }
        
        self.insert(newNode, at: index(at: beforeNode))
    }
